  ...
  return (
    <nav className="navbar">
      ...
      <div className="flex items-center gap-2 max-sm:flex-wrap">
        <Link href="/upload" className="primary-button w-fit max-sm:px-3 max-sm:py-1.5 max-sm:text-sm">
          Analyze Resume
        </Link>
        ...
        <Link href="/plans" className="rounded-full px-4 py-2 border border-gray-200 hidden sm:block hover:bg-gray-50 transition-colors">
          Pricing
        </Link>
        {auth.isAuthenticated && (
          <Link href="/resumes" className="rounded-full px-4 py-2 border border-gray-200 hidden sm:block">
            My Resumes
          </Link>
        )}

        {auth.isAuthenticated ? (
          <div className={`relative ${pathname === '/upload' ? 'max-sm:hidden' : ''}`}>
            <button
            ...
            {open && (
              <div className="absolute right-0 mt-2 w-64 sm:w-80 bg-white border border-gray-200 rounded-2xl shadow-lg p-4 z-50">
                ...
                <div className="pt-3">
                  <Link href="/resumes" className="text-sm text-blue-600 hover:underline" onClick={() => setOpen(false)}>
                ...
                <div className="mt-3 pt-3 border-t border-gray-100 flex flex-col gap-2">
                  <Link href="/wipe" className="text-sm text-red-600 hover:underline" onClick={() => setOpen(false)}>
                    Wipe App Data
                  </Link>
                  <button
